...
def step04_train_models(
    ...
    for window in HORIZONS:
        ...
        # ── SMOTE oversampling ──────────────────────────────────────
        if balance in ("smote", "both"):
            print(f"  Applying SMOTE oversampling...")
            t_smote = time.time()
            smote = SMOTE(random_state=42)
            X_train, y_train = smote.fit_resample(X_train, y_train)
            pos_after = int(y_train.sum())
            neg_after = int((1 - y_train).sum())
            print(f"    After SMOTE: {pos_after} pos / {neg_after} neg ({time.time()-t_smote:.1f}s)")
        ...
        if balance in ("smote", "both"):
            ...
            X_train_train, y_train_train = smote_val.fit_resample(X_train_train, y_train_train)
