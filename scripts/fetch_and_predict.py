...
def step03_feature_engineering_and_infer(eventos_df: pd.DataFrame, recent_days: int):
    ...
    # Save predictions
    for name in ["predictions_daily.parquet", "predictions_voy_redbus.parquet", "predictions_best_config.parquet"]:
    ...
    predictions_df.to_parquet(PREDICTIONS_DIR / "predictions_log.parquet", index=False)
    print(f"  Log: {PREDICTIONS_DIR / 'predictions_log.parquet'}")
